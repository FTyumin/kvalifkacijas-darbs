<footer class="bg-black border-t border-border py-5 px-8">
    <div class="px-5 sm:px-8 lg:px-28">
    <div class="grid lg:grid-cols-4 gap-8 mb-12 ">
        <div class="space-y-4">
        <div class="text-2xl text-white font-light tracking-wide">MOVIE PLATFORM</div>
        <p class="text-muted-foreground text-white text-sm leading-relaxed">
            Your trusted source for thoughtful film and television criticism.
        </p>
        </div>

        <div class="space-y-4">
        <h3 class="font-medium">Content</h3>
        <div class="space-y-2 text-sm text-white">
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Movie Reviews
            </a>
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            TV Show Reviews
            </a>
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Critics' Picks
            </a>
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Film Festivals
            </a>
        </div>
        </div>

        <div class="space-y-4">
        <h3 class="font-medium text-white">Community</h3>
        <div class="space-y-2 text-sm">
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Join Critics
            </a>
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Submit Review
            </a>
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Discussion Forums
            </a>
            <a href="#" class="block text-muted-foreground text-white hover:text-foreground transition-colors">
            Newsletter
            </a>
        </div>
        </div>

        <div class="space-y-4">
        <h3 class="font-medium text-white">Connect</h3>
        <div class="space-y-2 text-sm">
            <a href="#" class="block text-muted-foreground hover:text-foreground text-white transition-colors">
            Instagram
            </a>
            <a href="#" class="block text-muted-foreground hover:text-foreground text-white transition-colors">
            Twitter
            </a>
            <a href="#" class="block text-muted-foreground hover:text-foreground text-white transition-colors">
            Letterboxd
            </a>
            <a href="#" class="block text-muted-foreground hover:text-foreground text-white transition-colors">
            YouTube
            </a>
        </div>
        </div>
    </div>

    <div class="border-t border-border pt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-sm text-muted-foreground text-white">
        <p>&copy; 2025  All rights reserved.</p>
        <div class="flex gap-6">
        <a href="#" class="hover:text-foreground transition-colors text-white">
            Privacy Policy
        </a>
        <a href="#" class="hover:text-foreground transition-colors text-white">
            Terms of Service
        </a>
        <a href="#" class="hover:text-foreground transition-colors text-white">
            Contact
        </a>
        </div>
    </div>
    </div>
</footer>